...
fragment float4 FXAAOperationFragment(SingleInputVertexIO fragmentInput [[stage_in]],
                                      texture2d<float> inputTexture [[texture(0)]]) {
	constexpr sampler quadSampler;
	float2 frameBufSize = float2(static_cast<float>(inputTexture.get_width()), static_cast<float>(inputTexture.get_height()));

	float4 color = fxaa(inputTexture, quadSampler, fragmentInput.textureCoordinate, frameBufSize);
	return color;
}
